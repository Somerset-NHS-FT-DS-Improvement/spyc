...
def add_control_lines(fig, data):
    ...
    fig.add_trace(go.Scatter(
        x=data.index, y=data['CL'], mode='lines',
        line=dict(color='#66a182', dash='dash'), name='Center Line'
    ))
    fig.add_trace(go.Scatter(
        x=data.index, y=data['UCL'], mode='lines',
        line=dict(color='#d1495b', dash='dot'), name='Upper Control Limit'
    ))
    fig.add_trace(go.Scatter(
        x=data.index, y=data['LCL'], mode='lines',
        line=dict(color='#d1495b', dash='dot'), name='Lower Control Limit'
    ))
...
def plotly_chart(data, process_change_dates, process_change_dict, figure_title='SPC chart'):
    ...
    # Add process change annotations
    if process_change_dates:
        for date in process_change_dates:
            # Plotly used 'new text' label if None provided in annotation_text
            annotation_text = "" if not process_change_dict[date]['process_names'] else \
                process_change_dict[date]['process_names']
...
def combine_figures(figures, fig_names, process_change_dates, process_change_dict, save_to_html=False, file_name=None):
    ...
    for i, fig in enumerate(figures):
        ...
        buttons.append(
            dict(
                label=f"{fig_names[i]}",
                method="update",
                args=[{"visible": visibility}, {"title": fig.layout.title.text if fig.layout.title else f"Figure {i+1}"}]
            )
        )
        start_idx = end_idx
    ...
    combined_fig.update_layout(
        ...
        xaxis=figures[0].layout.xaxis,  # Set the xaxis from the first figure (to preserve range, etc.)
