...
class SPC:
    ...
    def __init__(self,
        ...
        self.fix_control_end_dt = fix_control_end_dt if fix_control_end_dt is not None else \
            self.data_in.index.max()
    ...
    def add_process_change_date(self,
        ...
        Inputs:
            ...
            - process_name (str, optional): A name for the process change. This can be used to identify different process
              changes.
        """
        ...
        if self.fix_control_end_dt == self.data_in.index.max() or self.fix_control_end_dt > fix_control_start_dt:
            most_recent_key = max(
                [pd.Timestamp(key) for key in self.control_line_dates.keys() if pd.Timestamp(key) < \
                 pd.Timestamp(change_date)], default=None
            )
    ...
    def calculate_spc(self, spc_name, spc_calc_func, rule_1=True, rule_2=False,
                      rule_3=False, rule_4=False, rule_5=False) -> None:
    ...
    def __calculate_control_lines(self, spc_calc_func):
        ...
        for idx, date_tuple in enumerate(self.__change_dates_list):
            ...
            if self.seasonal and not check_data_points_per_season:
                ...
                warnings.warn('Not enough data to build reliable estimate of mean for each season. Global mean will be '
                              'estimated until more data is added (control line calculation period is changed, or '
                              'more data collected)', UserWarning)
        ...
        cl_df['period_name'] = cl_df.index.map(
            lambda date: (self.control_line_dates.get(date, {}).get('process_names', np.nan)
                          if date != first_key else np.nan))
